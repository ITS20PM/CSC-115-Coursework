...
public class A7Tester {
	...
	public static void map1_test() {
		...
		displayResults(!d.containsKey("def"), "containsKey");
		displayResults(d.containsKey("abc"), "containsKey");
		d.clear();
		...
		displayResults(passed, "contains");
		
		//test clear method
		d.clear();
		displayResults(!d.containsKey("abc"), "clear");
	...
	public static void map2_test() {
		...
		//test size() and put() methods in BSTMap
		for (int i = 0; i < 10000; i++) {
			d.put(i,i);
		}
		displayResults(d.size() == 10000, "put + size");

		//test get() method in BSTMap
		boolean passed = true;
		for (int i = 0; i < 10000; i++) {
			try {
				d.get(i);
		...
		//test containsKey() method in BSTMap
		passed = true;
		for(int i = 0; i < 10000; i++){
			if(!d.containsKey(i)){
				passed = false;
